...
assert_eq $'gfxes\ngpu_classes\ngpu_source\nos_key\nrepo_slug\nmethod\nartifacts\ndriver_mode\nkernel_status\nkernel_target\nkernel_package' "$(install_plan_keys)" "install plan includes GPU policy and kernel preparation fields"
...
assert_eq "12" "${#INSTALL_PLAN[@]}" "resolved plan contains GPU policy, kernel fields, and optional product names"
